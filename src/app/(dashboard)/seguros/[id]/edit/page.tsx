...
import { GET_APOLICE_TYPE, GET_CATEGORIES, GET_INSURE_ID, GET_INSURE_TYPE, GET_INSURES, POST_INSURE_UNIQUE, PUT_INSURE_UNIQUE } from "@/app/API"
...
import { SheetGroupNew } from "../../new/components/Group"
...
export default function InsurePageNew({ params }: Props) {

    const resolvedParams = React.use(params);
    const [insureId, setInsureId] = useState(resolvedParams.id)
    ...
    const { data: insure, isPending: isLoadingInsure } = useQuery({
        queryKey: ["get-insure-id", insureId],
        queryFn: ({ queryKey }) => GET_INSURE_ID(queryKey[1])
    })
    ...
    useEffect(() => {
        if (insure) {
            ...
            setGroupsState(insure.option_groups)
        }
    }
        , [insure])
    ...
    async function handleSubmit(params: FormEvent) {
        ...
        await createInsure({ name, id: insureId, description, policy_type_ids: apolices.map(i => i.id), insurance_type_ids: insures.map(i => i.id), category_ids: categories.map(i => i.id), option_groups: grouposState })
    }
        ...
        <div className="flex flex-col gap-4">
            <div className="flex items-end justify-between gap-2">
                <div className=" flex flex-col items-start w-full justify-start max-w-lg r">
                    <h1 className="text-zinc-800 font-extrabold text-lg">Editar Seguro</h1>
                </div>
            </div>
...
function Loading() {
    return (
        <div className="w-full h-full flex items-center justify-center">
            <Loader2 className="animate-spin" width={48} />
        </div>
    )
}